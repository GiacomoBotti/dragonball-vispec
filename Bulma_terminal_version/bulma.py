...
def main() -> int:
    ap = argparse.ArgumentParser(
        description=(
            "1) Extract Gaussian16/ORCA/QChem lower-triangular Hessian and flatten to 1-column vector (D exponents)\n"
            "2) Generate Gaussian16 input file (.com) from XYZ using --opt or --freq\n"
            "3) Generate ORCA/QChem input file (.inp) from XYZ using --orca-freq --orca-opt --qchem-opt --qchem-opt-freq --qchem-freq\n"
            "4) Extract Gaussian16/ORCA/QChem last printed geometry in the Output FIle (No Hessian Extraction) using --extract-geo\n"
            "5) Generate Gaussian16/ORCA/QChem input file for DYNAMICS calculations from XYZ and velocity file using --dyn --orca-qmd --qchem-qmd\n"
            "6) Extract Gaussian16/ORCA/QChem trajectory from DYNAMICS output calculations. Use --parse-dyn --parse-orca-qmd --parse-qchem-qmd \n"
